...
function showEc() {
    axios.get(api_3 + 'ec').then((r) => {
        ...
        // console.log(datArr)
        barChart(datArr, 'µs/cm', "ค่าการนำไฟฟ้า (µs/cm)");
...
function createMarker(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        // marker.addTo(map);
        marker.bindPopup("รหัสบ่อ:\n" + data[i].station_id + "<br> บ่อสังเกตการณ์:" + data[i].station_name + "<br> ที่ตั้ง\nตำบล" + data[i].tambon + "\nอำเภอ" + data[i].amphoe + "\nจังหวัด" + data[i].province).openPopup();
        // 
...
function showTable(data) {
    ...
    $('#tab tbody').on('click', 'tr', function () {
        ...
        L.popup({ offset: [0, -27] })
            ...
            .setContent(`รหัสบ่อ: ${data.station_id} <br> บ่อสังเกตการณ์: ${data.station_name}`)
            .openOn(map);
...
//     var amphoe = $("#am").val()
//     var tambon = $("#tam").val()
//     $.get("http://localhost:3000/api/add/" + province + "/" + amphoe + "/" + tambon).done(r => {
//         console.log("ok");
//         $("#pr").val("")
//     })
